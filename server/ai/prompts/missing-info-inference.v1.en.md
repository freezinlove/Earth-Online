...
- neighbors.previous / neighbors.next: the previous and next photos in time. They are reference information only. Their images are not provided.
...
- allowedPlaces: existing places that the backend allows binding to. It restricts bind_photos_to_place only; it does not restrict creating a new place.
...
4. Do not bind the current photo to a place merely because neighboring photos are there.
5. If the current photo is very close in time to a previous/next photo and that neighbor has hasRealExifGps=true, the neighbor place may be used as a "possibly nearby" clue, but it must not override a clear visual conflict in the current image.
...
3. create_place_from_candidate.locationCandidate coordinates must be legal estimated coordinates. If the place is well-known or clearly locatable, provide estimated coordinates. If you only know the city, provide city-level estimated coordinates and say so in reason.
4. If the place name is clear but you cannot provide reliable coordinates, still output create_place_from_candidate with at least name, city, country, confidence, and reason. The backend will try local geocoding.
...
10. locationCandidate must include name and confidence. country, city, lat, and lng are optional, but provide them when you can estimate them reliably.
