...
前端 Vite 服务默认在 `5173`，本地 API 默认在 `8787`。可以通过 `EARTH_ONLINE_PORT` 修改 API 端口。

## 环境变量

`.env.example` 只包含空占位，不包含任何真实密钥。常用变量：

- `QWEN_CHAT_API_KEY` / `QWEN_EMBEDDING_API_KEY` / `QWEN_API_KEY`
- `ALIYUN_API_KEY`
- `OPENAI_API_KEY`
- `OPENROUTER_API_KEY`
- `SILICONFLOW_API_KEY`
...
离线反向地理编码数据不会提交到 Git。需要时运行：

```bash
npm run geodata:setup
```

生成的数据位于 `external/geodata/geonames.sqlite`，已被 `.gitignore` 忽略。
...
- `external/geodata/*.sqlite` 和 `external/geodata/downloads/`
...
已提交的地球贴图、预生成 globe 数据和可选 GeoNames 数据来源记录在 `THIRD_PARTY_NOTICES.md`。如果替换素材，记得同步更新来源和许可说明。
